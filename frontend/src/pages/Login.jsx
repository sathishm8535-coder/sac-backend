import { useState, useContext } from 'react';
import { useNavigate, Link } from 'react-router-dom';
import { AuthContext } from '../context/AuthContext';
import axios from 'axios';
import toast from 'react-hot-toast';
import { LogIn } from 'lucide-react';

const Login = () => {
  const [tab, setTab] = useState('student'); // 'student' | 'staff'
  const [studentForm, setStudentForm] = useState({ email: '', password: '' });
  const [staffForm, setStaffForm] = useState({ username: '', password: '' });
  const [loading, setLoading] = useState(false);
  const { login } = useContext(AuthContext);
  const navigate = useNavigate();

  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    try {
      const payload = tab === 'student' ? studentForm : staffForm;
      const { data } = await axios.post('/api/auth/login', payload);
      login(data.token, data.user);
      toast.success('Login successful!');

      const { role } = data.user;
      if (role === 'admin') navigate('/admin-dashboard');
      else if (role === 'staff') navigate('/staff-dashboard');
      else navigate('/student');
    } catch (error) {
      toast.error(error.response?.data?.message || 'Login failed');
    } finally {
      setLoading(false);
    }
  };

  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
        <div className="flex items-center justify-center mb-4">
          <LogIn className="w-12 h-12 text-indigo-600" />
        </div>
        <h2 className="text-3xl font-bold text-center text-gray-800 mb-6">Login</h2>

        {/* Tab switcher */}
        <div className="flex rounded-lg overflow-hidden border border-gray-200 mb-8">
          <button
            type="button"
            onClick={() => setTab('student')}
            className={`flex-1 py-2 text-sm font-semibold transition ${
              tab === 'student' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'
            }`}
          >
            Student
          </button>
          <button
            type="button"
            onClick={() => setTab('staff')}
            className={`flex-1 py-2 text-sm font-semibold transition ${
              tab === 'staff' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'
            }`}
          >
            Admin / Staff
          </button>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">
          {tab === 'student' ? (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Email</label>
              <input
                type="email"
                required
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                value={studentForm.email}
                onChange={(e) => setStudentForm({ ...studentForm, email: e.target.value })}
              />
            </div>
          ) : (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Username</label>
              <input
                type="text"
                required
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                value={staffForm.username}
                onChange={(e) => setStaffForm({ ...staffForm, username: e.target.value })}
              />
            </div>
          )}

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Password</label>
            <input
              type="password"
              required
              className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
              value={tab === 'student' ? studentForm.password : staffForm.password}
              onChange={(e) =>
                tab === 'student'
                  ? setStudentForm({ ...studentForm, password: e.target.value })
                  : setStaffForm({ ...staffForm, password: e.target.value })
              }
            />
          </div>

          <button
            type="submit"
            disabled={loading}
            className="w-full bg-indigo-600 text-white py-3 rounded-lg font-semibold hover:bg-indigo-700 transition disabled:opacity-50"
          >
            {loading ? 'Logging in...' : 'Login'}
          </button>
        </form>

        {tab === 'student' && (
          <p className="text-center text-gray-600 mt-6">
            Don't have an account?{' '}
            <Link to="/register" className="text-indigo-600 font-semibold hover:underline">Register</Link>
          </p>
        )}
      </div>
    </div>
  );
};

export default Login;
